...
classify_example(h_s_versicolor, Data(training_set_indicies(34), 1:4)) % versicolor

% Classify an example for Virginica
classify_example(h_s_virginica, Data(training_set_indicies(67), 1:4)) % Virginica
...
for i = testing_set_indices
    find_s_c_set{index} = [classify_example(h_s_setosa, Data(i, 1:4)), Data(i, 5)];
    find_s_c_ver{index} = [classify_example(h_s_versicolor, Data(i, 1:4)), Data(i, 5)];
    find_s_c_vir{index} = [classify_example(h_s_virginica, Data(i, 1:4)), Data(i, 5)];
    index = index + 1;
end
...
for i = testing_set_indices
    ...
    ce_c_set{index} = [classify_for_candidate_elimination(h_s_setosa, h_g_setosa, Data(i, 1:4)), Data(i, 5)];
    ce_c_ver{index} = [classify_for_candidate_elimination(h_s_versicolor, h_g_versicolor, Data(i, 1:4)), Data(i, 5)];
    ce_c_vir{index} = [classify_for_candidate_elimination(h_s_virginica, h_g_virginica, Data(i, 1:4)), Data(i, 5)];
...
for i = testing_set_indices
    ...
    lte_c_set{index} = [classify_for_hypot_list(h_setosa, Data(i, 1:4)), Data(i, 5)];
    lte_c_ver{index} = [classify_for_hypot_list(h_versicolor, Data(i, 1:4)), Data(i, 5)];
    lte_c_vir{index} = [classify_for_hypot_list(h_virginica, Data(i, 1:4)), Data(i, 5)];
    
    index = index + 1;
end

disp('Finished')
disp('Please see the find_s_, ce_ and lte_ variables for classification results.')
